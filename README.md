...
## ➡️ Section 8: WireGuard VPN Configuration for Secure Remote Access

### 🎯 Objective
To deploy a secure, lightweight WireGuard VPN on pfSense to provide encrypted remote access for marketing team members while maintaining network segmentation, Zero Trust principles, and full visibility.

This configuration allows remote users to securely connect over the Internet and access only authorized internal resources.

---

## 8.1 VPN Technology Selected
...
- Widely adopted for enterprise and SMB remote access

---

## 8.2 WireGuard Package Installation on pfSense

WireGuard was installed directly on pfSense using the package manager.
...
4. Verify WireGuard service is available under:
   VPN → WireGuard

---

## 8.3 WireGuard Tunnel Configuration

### Tunnel Creation:

A WireGuard tunnel was created on pfSense with the following settings:

- Interface Address (Tunnel IP):  
  `192.168.50.1/24`

- Listen Port:  
  `51820/UDP` (default WireGuard port)

- Public Key:  
  Auto-generated by pfSense

- Private Key:  
  Auto-generated by pfSense

- Pre-Shared Key:  
  Generated for additional security

### Purpose:

This tunnel serves as the VPN gateway for remote users.

---

## 8.4 WireGuard Interface Assignment

The WireGuard tunnel was assigned as a pfSense interface.
...
2. Assign the WireGuard tunnel as a new interface

3. Enable the interface and name it:
   `WG_VPN`

This allows firewall rules to be applied specifically to VPN traffic.

---

## 8.5 Firewall Rules for WireGuard

### WireGuard Interface Rules

On the `WG_VPN` interface:
...
- Rule: Allow VPN Traffic
  ...
  - Source: WG_VPN net
  - Destination: Any
  - Description: Allow VPN traffic (controlled by downstream rules)

This allows VPN clients to pass traffic, while segmentation is enforced by inter-VLAN rules.

---

### WAN Firewall Rule (VPN Inbound)
...
This allows external clients to establish VPN tunnels.

---

## 8.6 Dynamic Public IP Challenge & Dynamic DNS Solution

### Problem:

The ISP provides a dynamic public IP address, which changes periodically. This breaks VPN connectivity because remote clients cannot reliably reach the firewall.

---

### Solution: Dynamic DNS with DuckDNS

DuckDNS was used to provide a consistent hostname for VPN access.

### Steps:

1. Created a DuckDNS account and domain (e.g., companyvpn.duckdns.org)

2. Configured Dynamic DNS in pfSense:

   Services → Dynamic DNS

3. Provider: DuckDNS

4. Hostname: companyvpn.duckdns.org

5. Credentials: DuckDNS token

6. Interface: WAN

This allows pfSense to automatically update DuckDNS whenever the public IP changes.

---

## 8.7 VPN Client Configuration (Remote Users)

### WireGuard Client Installation

On the remote user system:

- Installed WireGuard client software
- Generated:
  - Public Key
  - Private Key
...
### Peer Configuration on pfSense

For each remote user, a peer was created in the WireGuard tunnel.
...
- Allowed IPs (Client VPN IP):
  `192.168.50.10/32`

- Description:
  Marketing Remote User

---

## 8.8 Client Configuration Details
...
- Endpoint:
  `companyvpn.duckdns.org:51820`

- Accessible Networks (Allowed IPs):
  - `192.168.10.0/24` (IT LAN - if required)
  - `192.168.30.0/24` (Marketing LAN)

This ensures the remote user can only reach authorized internal networks.

---

## 8.9 VPN Access Control & Segmentation

Even though VPN traffic is allowed on the WG_VPN interface, segmentation is enforced by:

- Inter-VLAN firewall rules
- Server LAN restrictions
- Zero Trust access policies

This ensures:

- VPN users cannot access Server LAN by default
- VPN users are restricted to business-required networks
- Administrative access is blocked

---

## 8.10 Logging & Monitoring

WireGuard connection events and firewall activity are logged and forwarded to Splunk via pfSense syslog.
...
- Auditing remote user activity

---

## Security & Business Outcomes

✅ Encrypted remote access over the Internet  
✅ Works with dynamic public IPs  
✅ Secure key-based authentication  
✅ Segmented VPN access  
✅ Logged and monitored VPN activity  
✅ Enterprise-grade remote access design  
<img width="1233" height="311" alt="Screenshot 2026-01-30 125543" src="https://github.com/user-attachments/assets/596200af-9c5f-4160-ba4f-935209cb3142" />
<img width="1279" height="756" alt="Screenshot 2026-01-30 125228" src="https://github.com/user-attachments/assets/eaa0ba94-d46a-4554-9016-63f4b3cf31f9" />
<img width="1427" height="817" alt="Screenshot 2026-01-30 123120" src="https://github.com/user-attachments/assets/46755b75-2e7a-4ffb-a850-5e2df678c197" />
<img width="1429" height="771" alt="Screenshot 2026-01-30 122430" src="https://github.com/user-attachments/assets/326ed919-1653-429b-9e9a-50ae58b4c567" />
<img width="589" height="249" alt="Screenshot 2026-01-30 122305" src="https://github.com/user-attachments/assets/55f8f701-c507-447b-a33a-57774ef7f6cc" />
<img width="1116" height="427" alt="Screenshot 2026-01-30 122133" src="https://github.com/user-attachments/assets/8f21637c-1aa6-4e45-b267-27fce4e93f8e" />
<img width="1436" height="895" alt="Screenshot 2026-01-30 121859" src="https://github.com/user-attachments/assets/88b49fb4-f39a-45b6-b7d0-a9b9f81ce9c4" />
<img width="1417" height="816" alt="Screenshot 2026-01-30 121702" src="https://github.com/user-attachments/assets/83970651-0587-41f9-81f7-229c67a5d8e7" />
<img width="1062" height="487" alt="Screenshot 2026-01-30 121648" src="https://github.com/user-attachments/assets/9afa7486-668a-4437-a96e-e1f4cd568d5c" />
<img width="1405" height="798" alt="Screenshot 2026-01-30 121608" src="https://github.com/user-attachments/assets/e533df4d-8c3d-4415-9c47-c572d2b0496b" />
<img width="1432" height="891" alt="Screenshot 2026-01-30 130710" src="https://github.com/user-attachments/assets/112cf593-104a-40d7-8c15-5b1aff8dc1a1" />
<img width="840" height="652" alt="Screenshot 2026-01-30 130311" src="https://github.com/user-attachments/assets/913d1908-2d8d-4af7-940e-c1dfd2774721" />

---

## Result

This WireGuard VPN implementation provides secure, resilient, and monitored remote access for remote marketing users, while maintaining strong network segmentation and Zero Trust principles.

It reflects real-world SMB and enterprise VPN deployment practices.
